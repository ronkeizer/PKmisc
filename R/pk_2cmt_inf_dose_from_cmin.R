...
#' @export
pk_2cmt_inf_dose_from_cmin <- function(cmin = 1, tau = 12, t_inf = 1,
                                       CL = 3, V = 30,
                                       Q = 2, V2 = 20) {
  ...
  # calculate:
  dose <- cmin * t_inf / (
    ((A/alpha) * (1-exp(-alpha*t_inf)) * exp(-alpha * (tau - t_inf)) / (1-exp(-alpha*tau)) ) +
      ((B/beta)  * (1-exp(-beta *t_inf)) * exp(-beta  * (tau - t_inf)) / (1-exp(-beta*tau)) )
  )
  return(dose)
}
